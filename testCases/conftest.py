...
def pytest_metadata(metadata):
    metadata["Project Name"] = "CredKart Test Automation Framework"
    metadata["Environment"] = "QA Environment"
    metadata["Tester"] = "Credence Automation"
...
def pytest_html_results_summary(prefix, summary):
    """
    Hooks into pytest-html to modify the summary section.
    """
    # Adding a custom prefix (appears at the top of the summary)
    prefix.extend([f"<p>Test Run Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>"])
    prefix.extend(["This is edited summary"])
